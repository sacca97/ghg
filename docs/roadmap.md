...
- [x] Queue management: navigate queued messages with arrow keys when input is empty, delete/backspace to remove before sending
...
- [x] Detachable live worker sessions: `/detach` authorizes clientless background continuation while streams and subagents execute; `ghg ps`, `ghg attach`, and `ghg stop` manage live background workers over local Unix sockets (plan.md Phase 3.7)
...
- [x] Explicit TUI planning workflow: `/plan [goal]` enters conversational read-only Plan mode with `smart`, and `/execute [plan]` runs the latest or supplied Markdown proposal with `fast` — `internal/agent/plan_mode.go`, `internal/tui/plan.go`; legacy structured plans remain export-compatible

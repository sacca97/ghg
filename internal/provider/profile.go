// Package provider loads reusable, non-secret provider profiles and resolves
...
// Capabilities are metadata for future adapter and model-selection decisions.
...
func validateProfile(profile *Profile) error {
	...
	default:
		return fmt.Errorf("profile %q has unknown auth.kind %q (want bearer, header, or none)", profile.ID, profile.Auth.Kind)
	}
	...
	case CatalogOpenAIModels, CatalogAnthropicModels, CatalogStatic, CatalogNone:
	default:

...
// AnthropicClient talks to the native Anthropic Messages API. Its fields
// mirror Client so the profile-selected auth, headers, retry budget, and HTTP
// transport remain configurable through BackendConfig.
...
// NewAnthropic creates a client for Anthropic's native Messages API. BaseURL
// is normally the profile endpoint ending in /v1; the adapter appends
// /messages and /models.
func NewAnthropic(baseURL, apiKey string) *AnthropicClient {
...
func (c *AnthropicClient) attempts() int {
	if c.MaxRetries > 0 {
		return c.MaxRetries
	}
...
func (c *AnthropicClient) setRequestHeaders(hr *http.Request) error {
	return applyRequestHeaders(hr, c.Headers, c.APIKey, c.AuthKind, c.AuthHeader)
}

// Stream is the callback-compatible convenience form for direct Anthropic
// client users. Backend callers should use the request-local EventSink path.
func (c *AnthropicClient) Stream(ctx context.Context, req Request, onText, onThink func(string)) (Message, Usage, error) {
...
func (c *AnthropicClient) Complete(ctx context.Context, req Request) (string, Usage, error) {
...
func newAnthropicRequest(req Request, stream bool) (anthropicRequest, error) {
	...
	for _, msg := range msgs {
		...
		default:
			return anthropicRequest{}, fmt.Errorf("llm: anthropic cannot translate message role %q", msg.Role)
		}
	}
	if len(wire.Messages) == 0 {
		return anthropicRequest{}, errors.New("llm: anthropic request needs at least one user or assistant message")
	}
...
func anthropicSystemBlocks(msg Message) ([]anthropicBlock, error) {
	...
	for _, part := range msg.Parts {
		...
		case "text":
			...
			textAdded = true
		case "image_url":
			return nil, errors.New("llm: anthropic system content cannot contain images")
		default:
			return nil, fmt.Errorf("llm: anthropic does not support content part %q in system content", part.Type)
		}
	}
	return blocks, nil
}
	...
	for _, part := range msg.Parts {
		...
		case "image_url":
			if part.ImageURL == nil {
				return nil, errors.New("llm: anthropic image part has no URL")
			}
			...
			blocks = append(blocks, raw)
		default:
			return nil, fmt.Errorf("llm: anthropic does not support content part %q", part.Type)
		}
	}
	return blocks, nil
}
...
func anthropicAssistantBlocks(msg Message) ([]json.RawMessage, error) {
	if len(msg.ProviderBlocks) > 0 {
		...
		for _, raw := range msg.ProviderBlocks {
			...
			if err := json.Unmarshal(raw, &header); err != nil || header.Type == "" {
				...
				return nil, fmt.Errorf("llm: invalid preserved anthropic block: %w", err)
			}
	...
	for _, call := range msg.ToolCalls {
		...
		if !json.Valid([]byte(input)) {
			return nil, fmt.Errorf("llm: anthropic tool %q has invalid JSON input", call.Function.Name)
		}
...
func appendAnthropicToolResult(messages *[]anthropicMessage, msg Message) error {
	if msg.ToolCallID == "" {
		return errors.New("llm: anthropic tool result has no tool_use id")
	}
	raw, err := marshalAnthropicBlock(anthropicBlock{
		Type:      "tool_result",
	...
	if len(*messages) == 0 || (*messages)[len(*messages)-1].Role != "assistant" {
		return fmt.Errorf("llm: anthropic tool result %q is not after an assistant tool_use", msg.ToolCallID)
	}
...
func anthropicTools(tools []Tool) ([]anthropicTool, error) {
	...
	for _, tool := range tools {
		...
		if name == "" {
			return nil, errors.New("llm: anthropic tool name is required")
		}
		...
		if !json.Valid(schema) {
			return nil, fmt.Errorf("llm: anthropic tool %q has invalid input schema", name)
		}
...
func anthropicImageBlock(rawURL string) (anthropicBlock, error) {
	if rawURL == "" {
		return anthropicBlock{}, errors.New("llm: anthropic image URL is empty")
	}
	...
	if !ok {
		return anthropicBlock{}, errors.New("llm: malformed image data URL")
	}
	header = strings.TrimPrefix(header, "data:")
	parts := strings.Split(header, ";")
	if len(parts) < 2 || parts[len(parts)-1] != "base64" || parts[0] == "" {
		return anthropicBlock{}, errors.New("llm: anthropic images require base64 data URLs")
	}
	if _, err := base64.StdEncoding.DecodeString(encoded); err != nil {
		return anthropicBlock{}, fmt.Errorf("llm: malformed image data URL: %w", err)
	}
	return anthropicBlock{
		Type: "image",
...
func marshalAnthropicBlock(block anthropicBlock) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("llm: marshal anthropic content block: %w", err)
	}
	return b, nil
}
...
func (c *AnthropicClient) endpoint(suffix string) (string, error) {
	...
	if base == "" {
		return "", errors.New("llm: anthropic base url is required")
	}

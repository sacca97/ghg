...
func testResolvedProvider(envVar string) models.Resolved {
...
func TestUpsertProviderKeyLiteralPreservesConfig(t *testing.T) {
	...
	if p.Name != "OpenCode Go" || p.Profile != "opencode" || p.BaseURL != "https://opencode.example/v1" || p.API != string(models.ProtocolOpenAIChatCompletions) {
...
func TestAnyProviderUsableIsProfileAware(t *testing.T) {
	...
	userDir := t.TempDir()
	writeProviderTestProfile(t, userDir, "local", models.AuthNone)
	profiles, err := models.Load(models.LoadOptions{UserDir: userDir})
	if err != nil {
		t.Fatal(err)
	}

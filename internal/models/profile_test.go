...
func TestLoadEmbeddedProfiles(t *testing.T) {
	...
	codex, ok := profiles.Lookup("codex-subscription")
	if !ok || codex.Protocol != ProtocolOpenAIResponses || codex.Auth.Kind != AuthCodexSubscription {
		t.Fatalf("codex-subscription profile: %+v", codex)
	}
	res, err := profiles.Resolve(Instance{Name: "codex-subscription", Profile: "codex-subscription"})
	if err != nil {
		t.Fatal(err)
	}

...
type responsesFlavor int

const (
	responsesPublicAPI responsesFlavor = iota
	responsesCodexSubscription
)
...
// newOpenAIResponses creates an OpenAI Responses adapter.
...
func (c *OpenAIResponsesClient) tryForceRefresh(ctx context.Context) bool {
	if c.Authorizer == nil {
		return false
	}
...
func (c *OpenAIResponsesClient) endpoint(suffix string) (string, error) {
	...
	if base == "" {
		return "", errors.New("models: openai responses base url is required")
	}
...
func newOpenAIResponsesRequest(req Request, stream bool, flavor ...responsesFlavor) (openAIResponsesRequest, error) {
	fl := responsesPublicAPI
	if len(flavor) > 0 {
		fl = flavor[0]
	}
	...
	if fl == responsesCodexSubscription {
		wire.Store = nil
		wire.PromptCacheKey = ""
	}
	...
	for _, msg := range msgs {
		...
		case "tool":
			...
			wire.Input = append(wire.Input, raw)
		default:
			return openAIResponsesRequest{}, fmt.Errorf("models: openai responses cannot translate message role %q", msg.Role)
		}
	}
	...
	if len(wire.Input) == 0 && wire.Instructions == "" {
		return openAIResponsesRequest{}, errors.New("models: openai responses request needs input or instructions")
	}
	return wire, nil
}
...
func responsesSystemText(msg Message) (string, error) {
	...
	for _, part := range msg.Parts {
		...
		default:
			return "", fmt.Errorf("models: openai responses system content does not support part %q", part.Type)
		}
	}
...
func responsesUserMessage(msg Message) (json.RawMessage, error) {
	...
	appendText := func(text string) error {
		...
		if err != nil {
			return fmt.Errorf("models: marshal openai responses text: %w", err)
		}
	...
	for _, part := range msg.Parts {
		...
		case "image_url":
			if part.ImageURL == nil || strings.TrimSpace(part.ImageURL.URL) == "" {
				return nil, errors.New("models: openai responses image part has no URL")
			}
			raw, err := json.Marshal(openAIResponsesInputImage{
				Type: "input_image", ImageURL: part.ImageURL.URL, Detail: "auto",
			})
			if err != nil {
				return nil, fmt.Errorf("models: marshal openai responses image: %w", err)
			}
			content = append(content, raw)
		default:
			return nil, fmt.Errorf("models: openai responses does not support content part %q", part.Type)
		}
	}
...
func responsesAssistantItems(msg Message) ([]json.RawMessage, error) {
	if len(msg.ProviderBlocks) > 0 {
		...
		for _, raw := range msg.ProviderBlocks {
			...
			if err := json.Unmarshal(raw, &header); err != nil || header.Type == "" {
				...
				return nil, fmt.Errorf("models: invalid preserved openai responses output item: %w", err)
			}
	...
	if msg.Content != "" || len(msg.ToolCalls) == 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("models: marshal openai responses assistant text: %w", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("models: marshal openai responses assistant message: %w", err)
		}
	...
	for _, call := range msg.ToolCalls {
		...
		if !json.Valid([]byte(args)) {
			return nil, fmt.Errorf("models: openai responses tool %q has invalid JSON arguments", call.Function.Name)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("models: marshal openai responses function call: %w", err)
		}
		items = append(items, raw)
	}
	return items, nil
...
func responsesToolResult(msg Message) (json.RawMessage, error) {
	if strings.TrimSpace(msg.ToolCallID) == "" {
		return nil, errors.New("models: openai responses tool result has no call_id")
	}
...
func openAIResponsesTools(tools []Tool) ([]openAIResponsesTool, error) {
	...
	for _, tool := range tools {
		...
		if name == "" {
			return nil, errors.New("models: openai responses tool name is required")
		}
		...
		if !json.Valid(schema) {
			return nil, fmt.Errorf("models: openai responses tool %q has invalid parameters", name)
		}
...
// AdapterProtocol reports the OpenAI Responses adapter selected by this client.
func (c *OpenAIResponsesClient) AdapterProtocol() Protocol { return ProtocolOpenAIResponses }
...
func (c *OpenAIResponsesClient) Complete(ctx context.Context, req Request) (Message, Usage, error) {
...
func (c *OpenAIResponsesClient) Models(ctx context.Context) ([]ModelInfo, error) {
	return c.doModels(ctx, true)
}

func (c *OpenAIResponsesClient) doModels(ctx context.Context, canRefresh bool) ([]ModelInfo, error) {
	...
	if err != nil {
		return nil, nonRetryable{fmt.Errorf("read openai responses models response: %w", err)}
	}

	if c.flavor == responsesCodexSubscription {
		var raw struct {
			Models *[]struct {
		...
		if err := json.Unmarshal(bodyBytes, &raw); err != nil {
			return nil, nonRetryable{fmt.Errorf("malformed codex models response: %w", err)}
		}
		if raw.Models == nil {
			return nil, nonRetryable{errors.New("unexpected codex models response: missing \"models\" array")}
		}
		...
		for _, m := range *raw.Models {
			...
			for _, l := range m.SupportedReasoningLevels {
				if eff := strings.TrimSpace(l.Effort); eff != "" {
	...
	if list.Data == nil {
		return nil, nonRetryable{errors.New("unexpected models response: missing \"data\" array")}
	}
...
func openAIResponsesHTTPError(resp *http.Response) error {
	...
	if err.Body == "" {
		...
		case http.StatusUnauthorized:
			err.Body = "ChatGPT session expired or revoked; please run 'ghg auth codex-subscription' to sign in again"
		case http.StatusForbidden:
			err.Body = "ChatGPT account lacks Codex access or workspace entitlement"
		}
	}
	return err
}

...
func TestAskPermissionUnblocksOnProgramExit(t *testing.T) {
	m := compactCmdModel()
	m.perms = permRules{}
	...
	go func() {
		decision, redirect = m.askPermission(tools.GateRequest{Tool: "bash", Command: "rm -rf /"})
		close(unblocked)
	}()
	...
	case <-unblocked:
		if decision != tools.GateReject {
			t.Fatalf("decision = %v, want GateReject", decision)
		}
		if redirect != "interactive reviewer closed" {
			t.Fatalf("redirect = %q, want 'interactive reviewer closed'", redirect)
		}
	case <-time.After(time.Second):
		t.Fatal("askPermission timed out waiting for program exit unblock")
	}
}
